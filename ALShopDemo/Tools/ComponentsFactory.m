...
+ (UILabel *)labelWithFrame:(CGRect)frame
    ...
    label.text = (NSNull *)text == [NSNull null] ? @"" : text;
					  ...
					  oriention:(BOOL)isBehind
{
	...
	if ([cycle caseInsensitiveCompare:@"D"] == NSOrderedSame) {
		...
		NSString* nextChartTimeStr = [[NSString alloc] initWithFormat:@"%d%@",iYear,strMonth] ;
...
+ (NSString*)noEmptyString:(NSString *)str
{
    return ([str isEqual:[NSNull null]] || str == nil)? @"" : str;
}

+ (NSString*)noEmptyStringReturn0:(NSString *)str
{
    return ([str isEqual:[NSNull null]] || str == nil)? @"0" : str;
}
...
+ (BOOL)getIsLogin{
    ...
    BOOL isLogin = [bus.rspUserInfo[@"expand"][@"isLogin"] boolValue];
    return isLogin;
}
...
+ (NSString*)showPrice:(NSString *)price {
    ...
    NSString *currentPrice = [ComponentsFactory showNewPrice:price];
    return [NSString stringWithFormat:@"¥ %@", currentPrice];
}

+ (NSString*)showNewPrice:(NSString *)price {
    
    NSString *currentPrice = nil;
    NSString *priceValue = [NSString stringWithFormat:@"%@" ,[ComponentsFactory noEmptyStringReturn0:price]];
    if ([priceValue intValue] %100 ==0 ) {
        currentPrice = [NSString stringWithFormat:@"%d", [priceValue intValue]/100];
    } else {
        currentPrice = [NSString stringWithFormat:@"%.2f", [priceValue doubleValue]/100];
    }
